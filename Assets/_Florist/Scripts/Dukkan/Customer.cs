...
public class Customer : MonoBehaviour
{
    ...
    public void SetCustomer(CustomerInfo customerInfo)
    {
        ...
        if (_customerInfo.OrderHappinessType == OrderHappinessType.Default)
        {
    ...
    public string Talk(string localizationKey, string defaultMessage, List<Option> answerOptions, List<StringParseOptions> parseOptions)
    {
        ...
        Debug.Log($"#customer# message: {message}");
        for (int i = 0; i < parseArgs.Length; i++)
        {
            Debug.Log($"#customer# parseArgs[i]: {parseArgs[i]}");
        }
    ...
    public string Talk(string localizationKey, string defaultMessage, Action onCompleted)
    {
        Debug.Log($"#customer# Talk called with localizationKey: {localizationKey} and defaultMessage: {defaultMessage}");
    ...
    public FlowerDeliveredInfo GetOrderInfo(List<BouquetModel> bouquetModels, EarningsInfo earningsInfo)
    {
        ...
        for (int i = 0; i < orderContentResults.Count; i++)
        {
            ...
            if (result.OrderSimilarity >= ORDER_SIMILARITY_LIMIT && result.IsRibbonCorrect && result.IsWrappingPaperCorrect)
            {
                ...
                happinessChange += _customerInfo.OrderHappiness.HappinessChange[HappinessState.SameOrder];
            }
            else
            {
                happinessState |= HappinessState.DifferentOrder;
                happinessChange += _customerInfo.OrderHappiness.HappinessChange[HappinessState.DifferentOrder];
            }

            if (result.ExtraFlowers.Count > 0)
            {
                happinessState |= HappinessState.MoreFlowers;
                happinessChange += _customerInfo.OrderHappiness.HappinessChange[HappinessState.MoreFlowers];
            }

            if (result.MissingFlowers.Count > 0)
            {
                happinessState |= HappinessState.MissingFlowers;
                happinessChange += _customerInfo.OrderHappiness.HappinessChange[HappinessState.MissingFlowers];
            }
        }
    ...
    public void OnSpeechEnd()
    {
        ...
        if (_showAnswersAfterSpeech)
        {
            _showAnswersAfterSpeech = false;

            int answerCount = _answerOptions == null ? 0 : _answerOptions.Count;
            for (int i = 0; i < answerCount; i++)
            {
                string buttonText = LocalizationManager.GetLocalizedText(_answerOptions[i].Message);
                _speechBubbleButtonImages[i].gameObject.SetActive(true);
                _speechBubbleButtonTexts[i].gameObject.SetActive(true);
                _speechBubbleButtonTexts[i].text = buttonText;

                _speechBubbleButtons[i].onClick.RemoveAllListeners();
                Action action = _answerOptions[i].Advance;
                _speechBubbleButtons[i].onClick.AddListener(() =>
                {
                    HapticsController.PlayButtonHaptic();
                    References.DukkanPage.AddToConvoHistory(buttonText);
                    action?.Invoke();
                });
            }
        ...
        for (int k = 0; k < bouquetCount; k++)
        {
            ...
            if (bouquetTypeToOrder == BouquetType.Custom)
            {
                ...
                for (int i = 0; i < flowerCount; i++)
                {
                    ...
                    {
                        FlowerType = order.CustomFlowers[i].FlowerType,
                        Count = order.CustomFlowers[i].Count,
                        FlowerColor = order.CustomFlowers[i].FlowerColor
                    };
    ...
    private List<OrderContentResult> CheckOrderContent(List<BouquetModel> receivedBouquetModels)
    {
        ...
        for (int k = 0; k < _bouquetsToOrder.Count; k++)
        {
            ...
            for (int i = 0; i < receivedBouquetModels.Count; i++)
            {
                ...
                // Group all flowers from receivedBouquetModels by FlowerType and FlowerColor, summing their counts
                var receivedFlowerCounts = receivedBouqet.Flowers
                    .GroupBy(f => new { f.FlowerType, f.FlowerColor })
                    .Select(g => new BouquetFlowerInfo
                    {
                        FlowerType = g.Key.FlowerType,
                        FlowerColor = g.Key.FlowerColor,
                        Count = g.Sum(f => f.Count)
                    })
                    .ToList();
                receivedBouqet.Flowers = receivedFlowerCounts;
    ...
    {
        for (int i = 0; i < _speechBubbleButtonImages.Count; i++)
        {
            _speechBubbleButtonImages[i].gameObject.SetActive(false);
            _speechBubbleButtonTexts[i].gameObject.SetActive(false);
        }
